...
const resolver = toTypedSchema(
  z.object({
    ...
    image: z.instanceof(File, { message: 'Image is required' }).refine((file: File) => file.size < 1000000, { message: 'Image must be less than 1MB' })
  })
);
...
<template>
  ...
  <div class="form-container form-container-common">
      <Card>
        ...
        <template #content>
          <Form v-slot="{ setFieldValue, values, errors }" :initialValues="initialValues" :resolver="resolver" @submit="onFormSubmit" class="form-container form-container-common">
            ...
            <div class="field-common">
              ...
              <Message v-if="errors.location" severity="error" size="small" variant="simple">{{ errors.location }}</Message>
            </div>
            <div class="field-common">
              <label for="image">Image</label>
              <FileUpload name="image" @select="onFileSelect($event, setFieldValue)" :multiple="false" accept="image/*" :maxFileSize="1000000" customUpload>
                  <template #empty>
            ...
            <div class="field-common">
              ...
              <Field name="description" v-slot="{ field }">
                <Editor :modelValue="field.value" @update:modelValue="field.onInput" editorStyle="height: 320px" />
              </Field>
